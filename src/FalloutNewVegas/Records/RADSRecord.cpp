...
uint32_t RADSRecord::GetType()
    {
    return REV32(RADS);
    }

char * RADSRecord::GetStrType()
    {
    return "RADS";
    }

int32_t RADSRecord::ParseRecord(unsigned char *buffer, unsigned char *end_buffer, bool CompressedOnDisk)
    {
    uint32_t subType = 0;
    uint32_t subSize = 0;
...
int32_t RADSRecord::WriteRecord(FileWriter &writer)
    {
    WRITE(EDID);
    WRITE(DATA);
