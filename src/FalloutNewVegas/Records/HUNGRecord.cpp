...
uint32_t HUNGRecord::GetType()
    {
    return REV32(HUNG);
    }

char * HUNGRecord::GetStrType()
    {
    return "HUNG";
    }

int32_t HUNGRecord::ParseRecord(unsigned char *buffer, unsigned char *end_buffer, bool CompressedOnDisk)
    {
    uint32_t subType = 0;
    uint32_t subSize = 0;
...
int32_t HUNGRecord::WriteRecord(FileWriter &writer)
    {
    WRITE(EDID);
    WRITE(DATA);
