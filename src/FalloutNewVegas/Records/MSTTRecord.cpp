...
uint32_t MSTTRecord::GetType()
    {
    return REV32(MSTT);
    }

char * MSTTRecord::GetStrType()
    {
    return "MSTT";
    }

int32_t MSTTRecord::ParseRecord(unsigned char *buffer, unsigned char *end_buffer, bool CompressedOnDisk)
    {
    uint32_t subType = 0;
    uint32_t subSize = 0;
...
int32_t MSTTRecord::WriteRecord(FileWriter &writer)
    {
    WRITE(EDID);
    WRITE(OBND);
    WRITE(FULL);
